...
parallel --ungroup --retries 0 --halt now,fail=1 ::: 'sudo service mariadb start' 'cd ~/cms/master_v_balashihe_graphql_api && code .' 'cd ~/cms/master_v_balashihe && sleep 0.3 && code .' 'cd ~/cms/master_v_balashihe_graphql_api && pnpm start:dev' 'cd ~/cms/master_v_balashihe && pnpm dev'
