...
import MasterSvgLogo from "@/ui/master-svg-logo";
import dynamic from "next/dynamic";
import PaletterModeSwitch from "@components/common/paletter/PaletteSwitch";
import { useShopProvider } from "@framework/commerce/use-shop-provider";
...
const Navbar: FC = () => {
  ...
  const { cart, cartEmpty } = useShopProvider();
  const { openSidebar } = useUI();
  const cartItemsCount = React.useMemo(() => {
    return !mounted || cartEmpty ? null : cart?.lineItems.length || null;
  }, [mounted, cart, cartEmpty]);
  React.useEffect(() => {
    setIsMounted(true);
  }, []);
      ...
      >
        ...
        >
          ...
          <Box>
            {!cartPage && (
              <StyledBadge
                badgeContent={cartItemsCount || null}
                max={999}
                anchorOrigin={{
                  vertical: "bottom",
                  horizontal: "left",
